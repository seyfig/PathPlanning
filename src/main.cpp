...
int main() {
  ...
  h.onMessage([&lane, &ref_v, &map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
          ...
          // j[1] is the data JSON object
            ...
            for (int i = 0; i < sf_size; i++) {
              ...
              if ((d < 2.0 + 4.0 * lane + 2.0) && (d > 2.0 + 4.0 * lane - 2.0)) {
                ...
                double check_car_s = ocar[5];

                // TODO MISSING POINT IN MPC solution,
                // you take the current position of other cars
                // not the points in the future
                // if using previous points can project s value out...
                check_car_s += (double)(pre_size * tint * check_speed);
                // check s values greater than the car and s gap
                if ((check_car_s > car_s) && (check_car_s - car_s < 30)){
                  // Do some logic here, lower reference velocity
                  // so we dont crash into the car infront of us, could ...
                  // ref_v = 29.5; //mph
                  too_close = true;
                  if (lane > 0) {
                    lane--;
            ...
            // TODO CHANGE it to every step
            // MPC no change required
            if (too_close) {
              ref_v -= 0.224;
            }
            else if (ref_v < 49.5) {
            ...
            if (option == 0) {
              ...
              if (pre_size < 2) {
                ...
                // VIDEO 25:00 - 27:00
                // TODO TEST, draw these points on simulator? in a graph
              ...
              for (int i = 0; i < 3; i++){
                ...
                vector<double> next_wp = getXY(car_s + 30 * (i + 1),(2.0 + 4.0 * lane), map_waypoints_s, map_waypoints_x, map_waypoints_y);
              ...
              for (int i = 1; i <= 50 - pre_size; i++) {
                ...
                double N = (target_dist/(tint * ref_v * mphtoms));
